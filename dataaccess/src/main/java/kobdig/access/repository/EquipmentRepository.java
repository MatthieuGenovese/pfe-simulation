...
public interface EquipmentRepository extends CrudRepository<EquipmentE, Integer> {
    ...
    @Query(value = "SELECT * FROM (SELECT * FROM Equipamentos e WHERE e.codigo_upz = 85 OR e.codigo_upz = 81 OR e.codigo_upz = 80 OR e.codigo_upz = 46 OR e.codigo_upz = 112 OR e.codigo_upz = 116 OR e.codigo_upz = 31 OR e.codigo_upz = 30 OR e.codigo_upz = 29 OR e.codigo_upz = 28 OR e.codigo_upz = 27) a INNER JOIN buffer b ON ST_Intersects(a.geom, b.geom) WHERE b.id_land = :id", nativeQuery = true)
    List<EquipmentE> findById(@Param("id")int id);
}
